...
public class RegistrationRepositoryImpl implements RegistrationRepository {
	...
	private static final String SELECT_PRE_SUBJECT_HISTORY = " SELECT " + "    s.id AS sub_id, " + "    s.name AS title, "
			+ "    p.name AS p_name, " + "    grades, " + "    sub_day, " + "    start_time, " + "    end_time, "
			+ "    room_id, " + "    capacity, " + "    num_of_student " + "    FROM " + "    sub_registration AS r "
			+ "        JOIN " + "    subject_tb AS s ON r.sub_id = s.id " + "        JOIN "
			+ "    professor_tb AS p ON s.professor_id = p.id " + "    WHERE " + "    stu_id = ? ";
	
	private static final String SELECT_SUBJECT_HISTORY = " SELECT " + "    s.id AS sub_id, " + "    s.name AS title, "
			+ "    p.name AS p_name, " + "    grades, " + "    sub_day, " + "    start_time, " + "    end_time, "
			+ "    room_id, " + "    capacity, " + "    num_of_student " + "    FROM " + "    sub_registration AS r "
			+ "        JOIN " + "    subject_tb AS s ON r.sub_id = s.id " + "        JOIN "
			+ "    professor_tb AS p ON s.professor_id = p.id " + "    WHERE " + "    stu_id = ? AND sub_id = ? ";
	...
	@Override
	public SubjectHistory resistrationHistory(int studentId, int subjectId) {
		...
		try (Connection conn = DBUtil.getConnection();
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				...
				while (rs.next()) {
					dto = SubjectHistory.builder().id(rs.getString("sub_id"))
							.title(rs.getString("title")).professorName(rs.getString("p_name"))
							.grades(rs.getString("grades")).subDay(rs.getString("sub_day"))
							.startTime(rs.getString("start_time")).endTime(rs.getString("end_time"))
							.roomId(rs.getString("room_id")).capacity(rs.getString("capacity"))
